...
class Trainer:
    ...
    def train(self, epochs=20, report_interval=1, plot_interval=20, rtol=1e-5, atol=1e-6, num_samples=5):
        ...
        for epoch in range(epochs):
            ...
            # with ef.scan():
            initial_value = torch.zeros((self.num_genes, 1), dtype=torch.float64)
            output = self.model(self.t_observed, initial_value, rtol=rtol, atol=atol, num_samples=num_samples)
            ...
            ll, kl = self.model.elbo(self.m_observed, output, mult)
            total_loss = -ll + kl
            total_loss.backward()
            self.optimizer.step()
